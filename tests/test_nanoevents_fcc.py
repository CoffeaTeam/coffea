...
def test_lorentz_behavior(delayed_events):
    ...
    assert delayed_events.ReconstructedParticles.behavior['LorentzVector'] == LorentzVectorRecord
    assert isinstance(delayed_events.Particle.eta.compute(), awkward.highlevel.Array)
    assert isinstance(delayed_events.ReconstructedParticles.eta.compute(), awkward.highlevel.Array)
