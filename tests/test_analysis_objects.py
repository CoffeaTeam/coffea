...
def test_analysis_objects():
    ...
    print('match mask: ',jca_reco.match(jca_gen,0.3) )
    print('arg matches: ',jca_reco.argmatch(jca_gen,0.3))
    argmatch_nocut = jca_gen.argmatch(jca_reco).flatten()
    argmatch_dr03  = jca_gen.argmatch(jca_reco,0.3).flatten()
    argmatch_dr03_dpt01 = jca_gen.argmatch(jca_reco,0.3,0.1).flatten()
