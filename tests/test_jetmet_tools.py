...
def test_factorized_jet_corrector():
    ...
    corrs_L1_jag_ref = ak.full_like(test_pt_jag, 1.)
    corrector = FactorizedJetCorrector(**{name: evaluator[name] for name in jec_names[0:1]})
    corrs_L1_jag = corrector.getCorrection(JetEta=test_eta_jag, Rho=test_Rho_jag, JetPt=test_pt_jag, JetA=test_A_jag)
    print("Reference L1 corrections:", corrs_L1_jag_ref)
    print("Calculated L1 corrections:", corrs_L1_jag)
    assert(ak.all(np.abs(ak.flatten(corrs_L1_jag_ref) - ak.flatten(corrs_L1_jag)) < 1e-6))
    ...
    corrs_L2_jag_ref = ak.unflatten(np.array([1.37038741364, 1.37710384514, 1.65148641108, 1.46840446827,
                                              1.1328319784, 1., 1.50762056349, 1.48719866989]), counts)
    corrector = FactorizedJetCorrector(**{name: evaluator[name] for name in jec_names[1:2]})
    ...
    assert(ak.all(np.abs(ak.flatten(corrs_L2_jag_ref) - ak.flatten(corrs_L2_jag)) < 1e-6))
    ...
    corrs_L3_jag = ak.full_like(test_pt_jag, 1.)
    ...
    assert(ak.all(np.abs(ak.flatten(corrs_L1L2L3_jag_ref) - ak.flatten(corrs_L1L2L3_jag)) < 1e-6))
...
def test_jet_resolution():
    ...
    counts, test_eta, test_pt = dummy_jagged_eta_pt()
    test_Rho = np.full_like(test_eta, 10.)
    ...
    test_eta_jag = test_eta_jag[0:3]
    test_Rho_jag = test_Rho_jag[0:3]
    test_Rho_jag = ak.concatenate([test_Rho_jag[:-1], [ak.concatenate([test_Rho_jag[-1,:-1], 100.])]])
    counts = counts[0:3]
    print("Raw jet values:")
    print("pT:", test_pt_jag)
    print("eta:", test_eta_jag)
    ...
    resos_jag_ref = ak.unflatten(np.array([0.21974642, 0.32421591, 0.33702479, 0.27420327, 0.13940689, 0.48134521, 0.26564994, 1.]), counts)
    resos_jag = reso.getResolution(JetEta=test_eta_jag, Rho=test_Rho_jag, JetPt=test_pt_jag)
...
def test_jet_correction_uncertainty():
    ...
    juncs_jag_ref = ak.unflatten(np.array([[1.053504214, 0.946495786], [1.033343349, 0.966656651], [1.065159157, 0.934840843],
                                           [1.033140127, 0.966859873], [1.016858652, 0.983141348], [1.130199999, 0.869800001],
                                           [1.039968468, 0.960031532], [1.033100002, 0.966899998]]), counts)
    juncs_jag = list(junc.getUncertainty(JetEta=test_eta_jag, JetPt=test_pt_jag))
...
def test_jet_correction_regrouped_uncertainty_sources():
    ...
    juncs_jag_ref = ak.unflatten(np.array([[1.119159088, 0.880840912], [1.027003404, 0.972996596], [1.135201275, 0.864798725],
                                           [1.039665259, 0.960334741], [1.015064503, 0.984935497], [1.149900004, 0.850099996],
                                           [1.079960600, 0.920039400], [1.041200001, 0.958799999]]), counts)
    juncs_jag = list(junc.getUncertainty(JetEta=test_eta_jag, JetPt=test_pt_jag))
    for i, (level, corrs) in enumerate(juncs_jag):
        if level != "Total":
            continue
...
def test_jet_resolution_sf():
    ...
    resosfs_jag_ref = ak.unflatten(np.array([[1.857, 1.928, 1.786], [1.084, 1.095, 1.073], [1.364, 1.403, 1.325],
                                             [1.177, 1.218, 1.136], [1.138, 1.151, 1.125], [1.364, 1.403, 1.325],
                                             [1.177, 1.218, 1.136], [1.082, 1.117, 1.047]]), counts)
    resosfs_jag = resosf.getScaleFactor(JetEta=test_eta_jag)
...
def test_jet_resolution_sf_2d():
    ...
    resosfs_jag_ref = ak.unflatten(np.array([[1.11904, 1.31904, 1.], [1.1432, 1.2093, 1.0771], [1.16633, 1.36633, 1.],
                                             [1.17642, 1.37642, 1.], [1.1808, 1.1977, 1.1640], [1.15965, 1.35965, 1.],
                                             [1.17661, 1.37661, 1.], [1.1175, 1.1571, 1.0778]]), counts)
    resosfs_jag = resosf.getScaleFactor(JetPt=test_pt_jag, JetEta=test_eta_jag)
    print("Reference Resolution SF (jagged):", resosfs_jag_ref)
    print("Resolution SF (jagged):", resosfs_jag)
    assert(ak.all(np.abs(ak.flatten(resosfs_jag_ref) - ak.flatten(resosfs_jag)) < 1e-6))
...
def test_corrected_jets_factory():
    ...
    from coffea.jetmet_tools import FactorizedJetCorrector, JetCorrectionUncertainty, JetResolution, JetResolutionScaleFactor
    ...
    def smear_factor(jetPt, pt_gen, jersf):
        return ak.full_like(jetPt, 1.) + (jersf[:, 0] - ak.full_like(jetPt, 1.)) * (jetPt - pt_gen) / jetPt

    test_gen_pt = ak.concatenate([corrected_jets.pt_gen[0, :-2], corrected_jets.pt_gen[-1, :-1]])
    ...
    test_jer = ak.concatenate([jets.jet_energy_resolution[0, :-2], jets.jet_energy_resolution[-1, :-1]])
    test_jer_sf = ak.concatenate([jets.jet_energy_resolution_scale_factor[0, :-2], jets.jet_energy_resolution_scale_factor[-1, :-1]])
    test_jec = ak.concatenate([corrs[0, :-2], corrs[-1, :-1]])
    test_corrected_pt = ak.concatenate([corrected_jets.pt[0, :-2], corrected_jets.pt[-1, :-1]])
    test_corr_pt = test_raw_pt * test_jec
    test_pt_smear_corr = test_corr_pt * smear_factor(test_corr_pt, test_gen_pt, test_jer_sf)
