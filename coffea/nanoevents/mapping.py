...
class ParquetSourceMapping(Mapping):
    _debug = False
    ...
    def _tree(self, uuid, treepath):
        key = "ParquetSourceMapping:" + tuple_to_key((uuid, treepath))
        try:
            return self._cache[key]
        except KeyError:
            pass
    ...
    def __getitem__(self, key):
        uuid, treepath, start, stop, nodes = ParquetSourceMapping.interpret_key(key)
        if ParquetSourceMapping._debug:
            print("Gettting:", uuid, treepath, start, stop, nodes)
        stack = []
        skip = False
        for node in nodes:
