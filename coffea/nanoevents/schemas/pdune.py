...
class PDUNESchema(BaseSchema):
    ...
    def _recursive_zip(self,forms, hierarchy,key, final_zip = False):
        ...
        for k, v in hierarchy.items():
            if isinstance(v, collections.abc.Mapping):
                #print('if:', k,v)
                forms[k] = self._recursive_zip(forms.get(k, {}),hierarchy.get(k,{}),k, True)
    ...
    def _type_dictionary_builder(self, branch_forms):
        ...
        V3Sets = [set( [b.split("_")[-1][:-1] for b in self._filter_branches(all_branches, "*%s"%V)   ]   ) for V in V3Var]
        V3Set = V3Sets[0].intersection(V3Sets[1],V3Sets[2])

        V4Sets = [set( [b.split("_")[-1][:-len(V)] for b in self._filter_branches(all_branches, "*%s"%V)   ]   ) for V in V4Var]
        ...
        for b in all_branches: 
            ...
            behavior = ""
            if b_end in V3Names: behavior ="ThreeVector"
            elif b_end in V4Names: behavior="FourVector"
    ...
    def _build_collections(self, branch_forms):
        ...
        branches = self._sort_branches(branch_forms.keys()) #sorted branches used to create nanoevent
        ...
        for key in branches:
            ...
            if sum( which_top_key ) == 0 : continue
            

            top_key=  "".join( [ l*w for l, w in zip( which_top_key, obj_lists ) ] )
            ...
            if behavior != "":
                ...
                sub_keys[-1] = last_key
                self.mixins[last_key] = "ThreeVector" if last_key.endswith("3D") else "LorentzVector"
                sub_keys.append(v)
            ...
            self._recursion(keys,ak_form, key_form_dict,0) # add form to end of dictionary

            self._recursion(keys[:-1],'obj', key_dict,0) # add string "obj" to end of dictionary
