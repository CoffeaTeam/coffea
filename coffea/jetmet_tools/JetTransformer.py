...
_signature_map = {"JetPt": "pt", "JetEta": "eta", "Rho": "rho", "JetA": "area"}
...
class JetTransformer(object):
    ...
    def __init__(self, jec=None, junc=None, jer=None, jersf=None):
        ...
        if not isinstance(jec, FactorizedJetCorrector):
            raise Exception(
                'JetTransformer needs a FactorizedJetCorrecter passed as "jec"'
        ...
        if junc is not None and not isinstance(junc, JetCorrectionUncertainty):
            raise Exception(
                '"junc" must be of type "JetCorrectionUncertainty"'
        ...
        if jer is not None and not isinstance(jer, JetResolution):
            raise Exception(
                '"jer" must be of type "JetResolution"' + " got {}".format(type(jer))
            )
        self._jer = jer

        if jersf is not None and not isinstance(jersf, JetResolutionScaleFactor):
            raise Exception(
                '"jersf" must be of type "JetResolutionScaleFactor"'
    ...
    def transform(self, jet, met=None, forceStochastic=False):
        ...
        **postcondition** : jet.pt, jet.mass, jet.p4 are updated to represent the corrected jet based on the input correction set.
        ...
        if met is not None:
            ...
            if not (
                isinstance(met["p4"], ArrayMethods)
                or isinstance(met["p4"], PtEtaPhiMassArrayMethods)
            ):
        ...
        if self._jer is not None and self._jersf is not None:
            ...
            ptGenJet = (
                np.zeros_like(jet.pt.content)
                if forceStochastic
                else jet.ptGenJet.content
            )
            ...
            jsmear_cen = np.where(
                ...
                1.0 + np.sqrt(np.maximum(jersf[:, 0] ** 2 - 1.0, 0)) * jersmear,
            )

            jsmear_up = np.where(
                doHybrid,
                1 + (jersf[:, 1] - 1) * (jet.pt.content - ptGenJet) / jet.pt.content,
                1.0 + np.sqrt(np.maximum(jersf[:, 1] ** 2 - 1.0, 0)) * jersmear,
            )

            jsmear_down = np.where(
                doHybrid,
                1 + (jersf[:, -1] - 1) * (jet.pt.content - ptGenJet) / jet.pt.content,
                1.0 + np.sqrt(np.maximum(jersf[:, -1] ** 2 - 1.0, 0)) * jersmear,
            )
            ...
            jsmear_up = np.where(
                jsmear_up * jet.pt.content < min_jet_pt, min_jet_pt_corr, jsmear_up
            )
            jsmear_down = np.where(
                jsmear_down * jet.pt.content < min_jet_pt, min_jet_pt_corr, jsmear_down
            )
            jsmear_cen = np.where(
                jsmear_cen * jet.pt.content < min_jet_pt, min_jet_pt_corr, jsmear_cen
            )

            # need to apply up and down jer-smear before applying central correction
            jet.add_attributes(
                pt_jer_up=jsmear_up * jet.pt.content,
        ...
        met["p4"].content._contents = TLorentzVectorArray.from_ptetaphim(
            np.sqrt(new_x ** 2 + new_y ** 2), 0, np.arctan2(new_y, new_x), 0
        ).content
            ...
            for name, _ in juncs:
                for shift in ["up", "down"]:
                    px = (
                        ...
                        - (
                            initial_p4.x
                            - jet["pt_{0}_{1}".format(name, shift)] * jets_cos
                        ).sum()
                    )
                    py = (
                        met["p4"].y
                        - (
                            initial_p4.y
                            - jet["pt_{0}_{1}".format(name, shift)] * jets_sin
                        ).sum()
                    )
