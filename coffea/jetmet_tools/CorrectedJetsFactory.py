...
_ONE_F32 = numpy.array(1., dtype=numpy.float32)
_ZERO_F32 = numpy.array(0., dtype=numpy.float32)
_JERSF_FORM = {
    "class": "NumpyArray",
    "inner_shape": [
        3
...
def rand_gauss(item):
    def getfunction(layout, depth):
        if (isinstance(layout, awkward.layout.NumpyArray)
            or not isinstance(layout, (awkward.layout.Content,
                                       awkward.partition.PartitionedArray)
           )):
            return lambda: awkward.layout.NumpyArray(numpy.random.normal(size=len(layout)).astype(numpy.float32))
        return None

    out = awkward._util.recursively_apply(
        awkward.operations.convert.to_layout(item),
        getfunction)
    assert out is not None
    return awkward._util.wrap(out, awkward._util.behaviorof(item))


def jer_smear(variation, forceStochastic, pt_gen, jetPt, etaJet, jet_energy_resolution, jet_resolution_rand_gauss, jet_energy_resolution_scale_factor):
    ...
    if isinstance(jetPt, awkward.highlevel.Array):
        def getfunction(layout, depth):
            if (isinstance(layout, awkward.layout.NumpyArray)
                ...
                return lambda: awkward.layout.NumpyArray(numpy.zeros_like(size=len(jetPt), dtype=numpy.float32))
            return None
        if forceStochastic:
            pt_gen = awkward._util.recursively_apply(
                awkward.operations.convert.to_layout(jetPt),
                getfunction)
...
class CorrectedJetsFactory(object):
    ...
    def build(self, jets, lazy_cache):
        ...
        if self.jec_stack.jec is not None:
            ...
            out['jet_energy_correction'] = self.jec_stack.jec.getCorrection(**jec_args, form=scalar_form, lazy_cache=lazy_cache)
        else:
            out['jet_energy_correction'] = awkward.without_parameters(awkward.ones_like(out[self.name_map['JetPt']]))

        # finally the lazy binding to the JEC
        init_pt = partial(awkward.virtual, operator.mul, args=(out["jet_energy_correction"], out[self.name_map['ptRaw']]), cache=lazy_cache)
        init_mass = partial(awkward.virtual, operator.mul, args=(out["jet_energy_correction"], out[self.name_map['massRaw']]), cache=lazy_cache)
        ...
        out[self.name_map['JetPt'] + '_jec'] = init_pt(length=len(out), form=scalar_form)
        out[self.name_map['JetMass'] + '_jec'] = init_mass(length=len(out), form=scalar_form)
        ...
        if self.jec_stack.jer is not None and self.jec_stack.jersf is not None:
            ...
            out['jet_energy_resolution'] = self.jec_stack.jer.getResolution(**jerargs, form=scalar_form, lazy_cache=lazy_cache)
            ...
            out['jet_resolution_rand_gauss'] = awkward.virtual(rand_gauss, args=(out[jer_name_map['JetPt']],), cache=lazy_cache, length=len(out), form=scalar_form)

            init_jerc = partial(awkward.virtual,
                                jer_smear,
                                args=(
                                    0,
            ...
            out['jet_energy_resolution_correction'] = init_jerc(length=len(out), form=scalar_form)

            init_pt_jer = partial(awkward.virtual, operator.mul, args=(out["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            init_mass_jer = partial(awkward.virtual, operator.mul, args=(out["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            out[self.name_map['JetPt']] = init_pt_jer(length=len(out), form=scalar_form)
            out[self.name_map['JetMass']] = init_mass_jer(length=len(out), form=scalar_form)

            out[self.name_map['JetPt'] + '_jer'] = init_pt_jer(length=len(out), form=scalar_form)
            out[self.name_map['JetMass'] + '_jer'] = init_mass_jer(length=len(out), form=scalar_form)

            # JER systematics
            jerc_up = partial(awkward.virtual,
                              jer_smear,
                              args=(1,
                                    self.forceStochastic,
                                    out[jer_name_map['ptGenJet']],
            ...
            up['jet_energy_resolution_correction'] = jerc_up(length=len(out), form=scalar_form)
            init_pt_jer = partial(awkward.virtual, operator.mul, args=(up["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            init_mass_jer = partial(awkward.virtual, operator.mul, args=(up["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            up[self.name_map['JetPt']] = init_pt_jer(length=len(out), form=scalar_form)
            up[self.name_map['JetMass']] = init_mass_jer(length=len(out), form=scalar_form)

            jerc_down = partial(awkward.virtual,
                                jer_smear,
                                args=(2,
                                      self.forceStochastic,
                                      out[jer_name_map['ptGenJet']],
            ...
            down['jet_energy_resolution_correction'] = jerc_down(length=len(out), form=scalar_form)
            init_pt_jer = partial(awkward.virtual, operator.mul, args=(down["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            init_mass_jer = partial(awkward.virtual, operator.mul, args=(down["jet_energy_resolution_correction"], out[jer_name_map['JetPt']]), cache=lazy_cache)
            down[self.name_map['JetPt']] = init_pt_jer(length=len(out), form=scalar_form)
            down[self.name_map['JetMass']] = init_mass_jer(length=len(out), form=scalar_form)
            out['JER'] = awkward.zip({'up': up, 'down': down}, depth_limit=1, with_name='JetSystematic')
        ...
        if self.jec_stack.junc is not None:
            ...
            for name, func in juncs:
                ...
                up = awkward.flatten(jets)
                up[self.name_map['JetPt']] = awkward.virtual(junc_smeared_val,
                                                         args=(out[f"jet_energy_uncertainty_{name}"], 0,
                                                               out[juncnames['JetPt']],
                                                               ),
                                                         length=len(out),
                                                         form=scalar_form,
                                                         cache=lazy_cache)
                up[self.name_map['JetMass']] = awkward.virtual(junc_smeared_val,
                                                           args=(out[f"jet_energy_uncertainty_{name}"], 0,
                ...
                down = awkward.flatten(jets)
                down[self.name_map['JetPt']] = awkward.virtual(junc_smeared_val,
                                                           args=(out[f"jet_energy_uncertainty_{name}"], 1,
                                                                 out[juncnames['JetPt']],
                                                                 ),
                                                           length=len(out),
                                                           form=scalar_form,
                                                           cache=lazy_cache)
                down[self.name_map['JetMass']] = awkward.virtual(junc_smeared_val,
                                                             args=(out[f"jet_energy_uncertainty_{name}"], 1,
                                                                   out[juncnames['JetMass']],
                                                                   ),
                                                             length=len(out),
                                                             form=scalar_form,
                                                             cache=lazy_cache)
                out[f'JES_{name}'] = awkward.zip({'up': up, 'down': down}, depth_limit=1, with_name='JetSystematic')
