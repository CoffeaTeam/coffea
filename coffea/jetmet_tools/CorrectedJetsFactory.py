...
class CorrectedJetsFactory(object):
    def __init__(self, name_map, jec_stack):
        ...
        if isinstance(jec_stack, list) and isinstance(jec_stack[-1], bool):
            ...
            type(jec_stack).__name__ == "JECStack"
            and type(jec_stack).__module__ == "coffea.jetmet_tools.JECStack"
        ):
            self.tool = "jecstack"
        else:
            raise TypeError(
                "jec_stack need to be either an instance of JECStack or a list containing correction-lib setup!"
            )
        ...
        if self.tool == "clib":
            self.jer_names = [
                name
                for name in self.jec_stack
                if isinstance(name, str) and ("Resolution" in name or "SF" in name)
            ]
            self.junc_names = [
                name
                for name in self.jec_stack
                if isinstance(name, str) and ("Uncertainty" in name)
            ]
            self.jec_names = [
                name
                for name in self.jec_stack
                if (name not in self.jer_names and name not in self.junc_names)
            ]
    ...
    def build(self, jets, lazy_cache):
        ...
        if has_junc:
            ...
            if self.tool == "jecstack":
                ...
                self.junc_names = [
                    junc_name.replace("Quad_", "").replace(
                        "UncertaintySources_AK4PFchs_", ""
                    )
                    + "_AK4PFchs"
                ...
                for junc_name in self.junc_names:
                    ...
                    inputs = get_corr_inputs(
                        jets=juncjets, corr_obj=sf, name_map=juncnames
                    )
                ...
                for unc_up, unc_down in uncvalues:
                    combined = awkward.Array(
                        [[up, down] for up, down in zip(unc_up, unc_down)]
                    )
            ...
            def build_variation(unc, jetpt, jetpt_orig, jetmass, jetmass_orig, updown):
                ...
                var_dict[jetpt] = awkward.virtual(
                    ...
                    args=(
                        awkward.to_numpy(
                            awkward.values_astype(unc, numpy.float32)
                        ),  # this is needed for the clib variation
                        updown,
                        jetpt_orig,
                    ),
                    length=len(out),
